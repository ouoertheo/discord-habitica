...
class HabiticaUserServiceTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        ...
        self.HABITICA_USER_NAME = "TEST_USERN_AME"

    def test_init_store(self):
        app_user_service = self.app_user_service
        app_user_id = "discord_user"
        app_user = app_user_service.create_app_user(app_user_id)
        ...
        self.assertTrue(app2.get_app_user(app_user_id=app_user_id).id, app_user_id)
        self.assertTrue(app2.get_habitica_user_link(app_user_id=app_user_id).api_user, self.HABITICA_API_USER)
        self.assertTrue(app2.get_habitica_user_link(app_user_id=app_user_id).api_token, self.HABITICA_API_TOKEN)

    def test_create_app_user(self):
        app = self.app_user_service
        app.create_app_user("discord_user")
        self.assertTrue(app.get_app_user(app_user_id="discord_user").id, "discord_user")
    ...
    def test_get_app_user(self):
        ...
        self.assertTrue(app.get_app_user(api_user="api_user").id, "api_user")
        self.assertTrue(app.get_app_user(app_user_id="discord_user").id, "discord_user")
    ...
    def test_delete_app_user(self):
        ...
        app.delete_app_user("discord_user")
    ...
    async def test_get_habitica_user_links(self):
        app_user1 = self.app_user_service.create_app_user("discord_user")
        app_user2 = self.app_user_service.create_app_user("discord_user2")
        app_user3 = self.app_user_service.create_app_user("discord_user4")
